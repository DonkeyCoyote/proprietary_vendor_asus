...
function enable_mic(){
	...
	echo "[INBOX_FAN][USER] enable_mic : PWM=$PWM, RPM=$RPM" > /dev/kmsg
}
...
function disable_mic(){
	...
	if [ "$fan_type" == "0" ]; then
		...
		PWM=`cat /sys/class/hwmon/Inbox_Fan/device/PWM`
		RPM=`cat /sys/class/hwmon/Inbox_Fan/device/RPM`
		echo "[INBOX_FAN][USER] userfan_type=$fan_type(off), PWM=$PWM, RPM=$RPM" > /dev/kmsg
	elif [ "$fan_type" == "1" ]; then
		echo 1 > /sys/class/hwmon/Inbox_Fan/device/inbox_user_type
		PWM=`cat /sys/class/hwmon/Inbox_Fan/device/PWM`
		RPM=`cat /sys/class/hwmon/Inbox_Fan/device/RPM`
		echo "[INBOX_FAN][USER] userfan_type=$fan_type(low), PWM=$PWM, RPM=$RPM" > /dev/kmsg
	elif [ "$fan_type" == "2" ]; then
		echo 2 > /sys/class/hwmon/Inbox_Fan/device/inbox_user_type
		PWM=`cat /sys/class/hwmon/Inbox_Fan/device/PWM`
		RPM=`cat /sys/class/hwmon/Inbox_Fan/device/RPM`
		echo "[INBOX_FAN][USER] userfan_type=$fan_type(medium), PWM=$PWM, RPM=$RPM" > /dev/kmsg
	elif [ "$fan_type" == "3" ]; then
		echo 3 > /sys/class/hwmon/Inbox_Fan/device/inbox_user_type
		PWM=`cat /sys/class/hwmon/Inbox_Fan/device/PWM`
		RPM=`cat /sys/class/hwmon/Inbox_Fan/device/RPM`
		echo "[INBOX_FAN][USER] userfan_type=$fan_type(high), PWM=$PWM, RPM=$RPM" > /dev/kmsg
		...
		echo "[INBOX_FAN][USER] csc_test fan : userfan_type=$fan_type, PWM=$PWM, RPM=$RPM" > /dev/kmsg
	fi
}
