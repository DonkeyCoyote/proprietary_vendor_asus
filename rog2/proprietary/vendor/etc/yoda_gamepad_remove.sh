...
if [ "$update_process" != "1" ]; then
	...
	echo "[Gamepad Remove] ROGGamepadRemove, Update process $update_process, fw_status3 $fw_status3," > /dev/kmsg

	fw_status4=`getprop vendor.asus.accy.fw_status4`
	echo "[Gamepad Remove] ROGGamepadRemove, Update process $update_process, fw_status4 $fw_status4," > /dev/kmsg
