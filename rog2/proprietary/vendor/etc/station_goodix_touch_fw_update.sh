...
tp_fw_9896=`cat $device_PATH/fw_9896_info`

echo "[EC_HID][TPFW] hwid $hwid station TP fw:$station_tp_fw phone TP fw:$phone_tp_fw" > /dev/kmsg


if [ "$hwid" == "0x3" ]; then
	if [ $station_tp_chip_ver == "9886" ];then # station 3 chip ver is 9896
	...
	if [ "$station_tp_fw" = "$tp_fw_9896" ];then
		echo "[EC_HID][TPFW] FW is new didn't upgrade!" > /dev/kmsg
	else
		echo "[EC_HID][TPFW] FW ver $station_tp_fw need upgrade to $tp_fw_9896 !" > /dev/kmsg
	...
	if [ "$station_tp_chip_ver" != "9886" ]; then # station 2 chip ver is 9886
		echo "[EC_HID][TPFW] Unreliable hwid skip update fw, touch ic is $station_tp_chip_ver" > /dev/kmsg
		setprop vendor.station.tp_fwupdate 2
		exit
	fi
...
if [ "$station_tp_chip_ver" == "9896" ]; then
	fw_verion_new=$tp_fw_9896
else
