...
cp /vendor/factory/sensors/grip_panel_id /asdf/
PID_PATH="/asdf/grip_panel_id"
...
##### Check Panel ID #####
if [ -f $PID_PATH ]; then
    setprop vendor.grip.ID_orig 0
    setprop vendor.grip.ID_now 0
    setprop vendor.grip.panel_file "exist"
    Panel_ID_now="`cat /proc/lcd_unique_id`"
    Panel_ID_orig="`cat $PID_PATH`"
    setprop vendor.grip.ID_orig $Panel_ID_orig
    setprop vendor.grip.ID_now $Panel_ID_now
